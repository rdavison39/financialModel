"""
Tests for bulk brokerage directory imports.
"""
...
def test_import_bmo_directory(session, tmp_path):
    ...
    assert session.query(HoldingSnapshot).count() == 7
    assert session.query(CashSnapshot).count() == 2
    assert session.query(ImportRecord).count() == 1


def test_import_nesbitt_directory(session, tmp_path):
    """Import all Nesbitt Burns Excel files from a directory."""
    ...
    brokerage = session.query(Brokerage).one()
    assert brokerage.name == "Nesbitt Burns"

    account = session.query(Account).one()
    assert account.account_number == "70558197"

    assert session.query(HoldingSnapshot).count() == 50
    assert session.query(CashSnapshot).count() == 2
    assert session.query(ImportRecord).count() == 1
...
def test_empty_directory(session, tmp_path):
    """An empty directory should produce an empty result."""
