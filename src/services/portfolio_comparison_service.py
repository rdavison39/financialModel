...
class PortfolioComparisonService:
    ...
    def compare(self, first_date: date, second_date: date,
                account_id: int | None = None) -> PortfolioComparison:
        first_value = self._get_portfolio_value(first_date, account_id)
        second_value = self._get_portfolio_value(second_date, account_id)
        difference = (
        ...
        if account_id is None:
            first = self._get_consolidated_positions(first_date)
            second = self._get_consolidated_positions(second_date)
        else:
            first = self._get_positions(account_id, self._get_snapshot_date(account_id, first_date))
            second = self._get_positions(account_id, self._get_snapshot_date(account_id, second_date))
    ...
    def _get_portfolio_value(self, requested_date: date,
                             account_id: int | None) -> Decimal | None:
        ...
        return None if row is None else Decimal(str(row.total_value))
